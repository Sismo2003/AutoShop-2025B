...
import { getCustomersTableThunk } from "slices/thunk"
...
export default function UsersDatatable() {
  ...
  const { customers_table_view, loading } = useSelector((state) => state.customer);
  ...
  useEffect(() => {
    // Fetch users data from the Redux store
    if (customers_table_view.length > 0) {
      setUsers(customers_table_view);
    } else {
      // If no data is available, you can set an empty array or fetch from an API
      setUsers([]);
    }
  },[customers_table_view]);
  
  useEffect(() => {
    // Fetch users data from the Redux store
    dispatch(getCustomersTableThunk());
  }, [dispatch]);
